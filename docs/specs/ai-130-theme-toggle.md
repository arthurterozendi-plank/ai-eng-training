...
| 8   | ~~`window.matchMedia` is stubbed in tests rather than guarded in `resolveInitialTheme`~~ | **Overturned in review round 2.** The assumption was that guarding a call every browser supports would be testing the guard. It missed the blast radius: `resolveInitialTheme` runs inside `useState`'s lazy initializer in `ThemeToggle`, which the root layout renders with no error boundary above it, so a throw fails the entire page — a worse outcome than the unstyled render AC 4 anticipates. Both paths now guard `matchMedia`. |
...
**Round 1.** Three lanes ran against the committed diff: a general adversarial reviewer, the
repository's `review-security` lane, and its `review-conventions` lane. All three returned
`VERDICT: APPROVED`; no BLOCKING finding. Six NITs were adopted anyway, two of them because they
...
**Round 2** (`review-bug-hunter`, against the round-1 fixes) returned **`VERDICT:
CHANGES_REQUESTED`** with one BLOCKING finding, and it was correct:

> `resolveInitialTheme()` calls `window.matchMedia(...)` with no `try`/`catch`, while
> `THEME_INIT_SCRIPT` wraps the equivalent call in one. `resolveInitialTheme` is the lazy
> initializer for `useState` in `ThemeToggle`, which `RootLayout` renders with no error boundary
> above it — so a throw does not just break the toggle, it fails the whole page render. Every case
> in the eight-case table stubs `matchMedia`, so the table could never catch it.

**Adopted.** This overturns assumption #8 rather than merely amending it. `prefersDarkTheme()` now
guards the call, and the init script was restructured so that _each_ fallible read has its own
`try` and the apply always runs — storage and `matchMedia` fail independently, and one shared
`try` would abort on either. Three cases were added to the table (`matchMedia` missing, with and
without a stored value, and with storage blocked as well) plus a direct regression test asserting
`resolveInitialTheme` does not throw. All four were confirmed to fail against the unguarded code
before the fix landed.

The lesson worth carrying: "every browser supports this API" is an argument about _likelihood_,
and it does not survive contact with a _blast radius_ argument. The guard was cheap; the failure
it prevents is the whole page.
